...
internal class MCH
{
    ...
    internal class MCH_ST_DPS : CustomComboBase
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            ...
            {
                if (IsEnabled(Presets.MCH_ST_Reassemble) && ActionReady(Reassemble) && !InCombat()
                ...
                if (IsEnabled(Presets.MCH_ST_HeatBlast) && ActionReady(OriginalHook(Heatblast)) && HasEffect(Buffs.Overheated))
                {
    ...
    internal class MCH_AoE_DPS : CustomComboBase
    {
        ...
        protected override uint Invoke(uint actionID)
        {
            if ((actionID is SpreadShot or Scattergun or AutoCrossbow) && IsEnabled(Presets.MCH_AoE_DPS))
            {
                ...
                if (IsEnabled(Presets.MCH_AoE_Bioblaster) && ActionReady(Bioblaster)
                    ...
                    && (!TargetHasEffect(Debuffs.Bioblaster) || TargetEffectRemainingTime(Debuffs.Bioblaster) < 3))
                {
                    return Bioblaster;
                }

                if (IsEnabled(Presets.MCH_AoE_Chainsaw) && ActionReady(Chainsaw)
    ...
    internal class MCH_GaussRoundRicochet : CustomComboBase
    {
    ...
    internal class MCH_DismantleProtect : CustomComboBase
    {
