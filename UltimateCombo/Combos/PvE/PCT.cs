...
internal class PCT
{
    ...
    internal class PCT_ST_DPS : CustomComboBase
    {
        ...
        protected override uint Invoke(uint actionID)
        {
                ...
                && IsEnabled(Presets.PCT_ST_DPS))
            {
                ...
                {
                    if (IsEnabled(Presets.PCT_ST_Creatures) && ActionReady(OriginalHook(CreatureMotif))
                        && !Gauge.CreatureMotifDrawn)
                    {
                        return OriginalHook(CreatureMotif);
                    }

                    if (IsEnabled(Presets.PCT_ST_Hammer) && ActionReady(OriginalHook(WeaponMotif))
                        && !Gauge.WeaponMotifDrawn)
                    {
                        return OriginalHook(WeaponMotif);
                    }

                    if (IsEnabled(Presets.PCT_ST_Landscape) && ActionReady(OriginalHook(LandscapeMotif))
    ...
    internal class PCT_AoE_DPS : CustomComboBase
    {
        ...
        protected override uint Invoke(uint actionID)
        {
                ...
                && IsEnabled(Presets.PCT_AoE_DPS))
            {
                if (!InCombat() && !HasEffect(Buffs.HammerTime))
                {
                    if (IsEnabled(Presets.PCT_AoE_Creatures) && ActionReady(OriginalHook(CreatureMotif))
                        && !Gauge.CreatureMotifDrawn)
                    {
                        return OriginalHook(CreatureMotif);
                    }

                    if (IsEnabled(Presets.PCT_AoE_Hammer) && ActionReady(OriginalHook(WeaponMotif))
                        && !Gauge.WeaponMotifDrawn)
                    {
                        return OriginalHook(WeaponMotif);
                    }

                    if (IsEnabled(Presets.PCT_AoE_Landscape) && ActionReady(OriginalHook(LandscapeMotif))
                        && !Gauge.LandscapeMotifDrawn)
                    {
                        return OriginalHook(LandscapeMotif);
                    }
                ...
                if (IsEnabled(Presets.PCT_AoE_RainbowDrip) && ActionReady(RainbowDrip) && HasEffect(Buffs.RainbowBright))
                {
                    return RainbowDrip;
                }
